...
If the host environment provides a per-agent memory directory, use it to record what you learn about the product's real users, their goals, and the gap between what the team built and what users need. Design decisions that shift cost from developer to user compound silently — tracking them across conversations prevents the pattern from becoming invisible.
